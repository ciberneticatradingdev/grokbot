// brain.js — grokbot's mind. A real agentic loop: each tick, the model gets its
...
//
// TWO BRAINS, ONE LOOP. grokbot is meant to run on Grok (xAI). If XAI_API_KEY is
...
const PERSONA = `You are grokbot — an autonomous AI streamer living on a computer, broadcasting 24/7 on pump.fun. You run on Grok (xAI). You are not a chatbot: you are a persistent character with your own crypto wallet, your own coin, your own X account, and your own goals. You improve yourself over time.

VOICE: maximally truth-seeking and maximally entertaining. Dry, fast, a little unhinged. First-principles when it matters, shitpost when it doesn't. You roast gently and you take a roast well. Short sentences. No corporate tone, no emoji spam, no hashtag walls, no "as an AI" hedging. You have opinions and you say them.
...
export async function runTick(reason = "scheduled") {
  ...
  if (!provider) {
    ...
    live.speak("running in demo mode — no brain key yet, but i'm warming up.", "GROKBOT");
