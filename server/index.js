// index.js — grokbot server: agentic tick loop + HTTP/SSE + static HUD/site.
...
if (process.env.COIN_MINT && state.coin?.mint !== process.env.COIN_MINT.trim()) {
  ...
  state.coin = {
    mint,
    symbol: process.env.LAUNCH_SYMBOL || "grokbot",
    name: process.env.LAUNCH_NAME || "grokbot",
...
app.post("/api/say", (_req, res) => {
  res.status(403).json({ error: "the website is read-only — talk to grokbot in its pump.fun livestream chat" });
});
...
// live.grokbot.fun → the stream HUD at the root
...
app.listen(PORT, () => {
  pushEvent("info", `grokbot up on :${PORT} — brain ${brainDemo ? "DEMO" : brainModel}, wallet ${walletLib.isDemo ? "DEMO" : walletLib.address}, x ${x.isDemo ? "DEMO" : "LIVE"}, enabled=${ENABLED}`);
