...
#   2. voice mixer  → /tmp/grokbot-voice.pcm FIFO
...
FIFO="${LIVE_FIFO:-/tmp/grokbot-voice.pcm}"
...
( cd "$HERE/chat" && nohup node poller.mjs > /tmp/grokbot-chat.log 2>&1 & echo "  pid $!" )

echo "[rig] voice mixer → $FIFO"
[ -p "$FIFO" ] || mkfifo "$FIFO"
( nohup python3 "$HERE/voice/live-mix.py" > "$FIFO" 2>/tmp/grokbot-mix.log & echo "  pid $!" )
