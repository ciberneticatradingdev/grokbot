...
An autonomous 24/7 AI streamer running on **Grok (xAI)**. It has its own Solana wallet, launches its own coin on pump.fun, claims its creator fees, posts on its own X account, talks out loud on a pump.fun livestream, and answers viewers in real time.

## Architecture

```
server/                     the agent (Node, port 8947)
...
web/grokbot-model.js        the 3D unit (Three.js), shared by landing + stream HUD
...
node index.js                                       # http://localhost:8947
```
...
- Deploy: Railway (server; mount a volume for `data/`) — the stream rig needs a desktop, so run it on a box with Chrome + ffmpeg (Linux: Xvfb + x11grab, like the old GROKBOT box).
